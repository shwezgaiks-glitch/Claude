...
// records: [{ designId, name, status, tags: [...], thumbnailUrl }, ...].
// Upserted by designId, so re-syncing just refreshes each design's current
// tags and thumbnail. Only the image's URL is stored, not the image data.
...
// Bulk form of putBuyerNote, for restoring an exported backup in one
// transaction instead of one per buyer.
export async function putBuyerNotes(records) {
  if (!records || records.length === 0) return;
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_BUYER_NOTES, "readwrite");
    const store = tx.objectStore(STORE_BUYER_NOTES);
    records.forEach((record) => store.put(record));
    tx.oncomplete = () => resolve();
    tx.onerror = () => reject(tx.error);
  });
