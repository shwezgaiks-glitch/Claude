...
var SpoonflowerParser = (function () {
  ...
  function parseYearlyCsvStatement(text) {
    ...
    for (var r = 1; r < rows.length; r++) {
      ...
      // Payout rows were slipping into the transaction table as negative
      // "sales" with no design and a guest buyer, because this only trusted
      // the Type column and Spoonflower doesn't always spell it "payout"
      // there. Their Description is unambiguous ("Spoondollar Payout for
      // August 26, 2026"), so it decides — same as isReturn above overriding
      // a cancellation that still carries Type "sale".
      var isPayout = typeRaw === "payout" || /spoondollar payout|^payout\b/i.test(description);
      var type = isPayout ? "payout" : isReturn ? "return" : isFillAYard ? "fill_a_yard" : typeRaw === "sale" ? "sale" : typeRaw || "unknown";
